...
void ChunkMesh::markToRecompute() {
  mustRecompute = true;
}
...
bool ChunkMesh::hasSolidData() const {
  return solidOffset != 0;
}

bool ChunkMesh::hasTransparentData() const {
  return solidOffset != transpOffset.z;
}

bool ChunkMesh::hasData() const {
  return transpOffset.z != 0;
}

void ChunkMesh::drawSolid() {
...
void ChunkMesh::drawTransparent(vec3 dir) {
  ...
  if(any(greater) || any(lesser)) {
    ...
    for(int i = 0; i < 2*2*2; i++) {
      ...
      if(auto neigh = neighbors[(size_t)index - 1].lock()) {
        std::static_pointer_cast<ChunkImpl>(neigh)->markToRecompute();
      }
    }
  }
}
